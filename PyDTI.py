...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "m:d:f:c:s:o:n:p:g:r:i:w", ["method=","dataset=","data-dir=","cvs=","specify-arg=","method-options=","predict-num=","gpmi=","output-dir=","seed=","job-id=","workdir="])
...
#    seeds = [7771, 8367, 22, 1812, 4659]
    ...
    if sp_arg == 0 and predict_num == 0:
        if method == 'nrlmf':
            cv_eval.nrlmf_cv_eval(method,dataset,cv_data,X,D,T,cvs,args,logger,scoring='auc',gpmi=gpmi)
        if method == 'netlaprls':
    ...
    if sp_arg == 1 or predict_num > 0:
        ...
        if predict_num == 0:
            ...
            logger.info(cmd+', '+"auc:%.6f, aupr: %.6f, auc_conf:%.6f, aupr_conf:%.6f, Time:%.6f" % (auc_avg, aupr_avg, auc_conf, aupr_conf, time.clock()-tic))
